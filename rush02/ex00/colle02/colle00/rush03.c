...
/*   Updated: 2017/07/23 11:31:04 by clegirar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*stock_lines_03(t_par par, int a, int x, int y)
{
	if ((par.i == 1 && par.j > 1 && par.j < x)
		...
		par.buf[a] = 'B';
			...
			|| (par.i == y && par.j == 1))
		par.buf[a] = 'A';
	else if ((par.i == 1 && par.j == x) || (par.i == y && par.j == x))
		par.buf[a] = 'C';
	else
		par.buf[a] = ' ';
	return (par.buf);
...
char	*rush03(int x, int y)
{
	...
	while (par.i <= y)
	{
		...
		while (par.j <= x)
		{
			par.buf = stock_lines_03(par, a, x, y);
			a++;
			par.j = par.j + 1;
		}
		par.buf[a++] = '\n';
